...
public class ShaderUniformSettings {
    ...
    let shaderUniformSettingsQueue = DispatchQueue(
        label: "io.greedydev.MetalImageFramework.ShaderUniformSettingsQueue",
        attributes: [])
    ...
    func appendBufferSpace(for dataType:MTLDataType) {
        ...
        switch dataType {
            ...
            case .float3: uniformSize = 4 // Fix for alignment issue
    ...
    public func restoreShaderSettings(renderEncoder:MTLRenderCommandEncoder) {
        shaderUniformSettingsQueue.sync {
            ...
            let uniformBuffer = MetalRenderingDevice.shared.device.makeBuffer(
                bytes: uniformValues,
...
extension Color {
    func toFloatArray() -> [Float] {
        return [self.red, self.green, self.blue, 0.0]
    }
