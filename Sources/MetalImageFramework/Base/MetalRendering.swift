...
extension MTLCommandBuffer {
    func clear(with color: Color, outputTexture: Texture) {
        ...
        renderPass.colorAttachments[0].clearColor = MTLClearColorMake(Double(color.red), Double(color.green), Double(color.blue), Double(color.alpha))
    ...
    func renderQuad(
        ...
        let vertexBuffer = MetalRenderingDevice.shared.device.makeBuffer(bytes: imageVertices,
        ...
        for textureIndex in 0..<inputTextures.count {
            ...
            let textureBuffer = MetalRenderingDevice.shared.device.makeBuffer(bytes: inputTextureCoordinates,
...
) -> (MTLRenderPipelineState, [String: (Int, MTLDataType)]) {
